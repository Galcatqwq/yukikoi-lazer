        ...
        private void assertMatchesPerfectCircle(SliderPath path)
        {
            ...
            for (int i = 0; i < Math.Min(subpoints, 100); i++)
            {
                ...
                Assert.True(Precision.AlmostEquals(circularArcProperties.Centre + vector, path.PositionAt(progress), 0.01f),
                    "A perfect circle with points " + string.Join(", ", path.ControlPoints.Select(x => x.Position)) + " and radius" + circularArcProperties.Radius
                    + "from SliderPath does not almost equal a theoretical perfect circle with " + subpoints + " subpoints"
